...
const PORT = 8888;
const mongoDBUrl = "mongodb+srv://admin:3989302As@cluster0.o2m8r.mongodb.net/prof_orientation_test_db?retryWrites=true&w=majority";
...
mongoose.connect(mongoDBUrl, connectionOptions).then(
    () => {
...
app.get('/', (req, res) => {
    Question.find().exec().then(
        (questions) => {
            ...
            catA = questionsFromJson.filter(question => question.question_category == "A")
            catB = questionsFromJson.filter(question => question.question_category == "B")
            catC = questionsFromJson.filter(question => question.question_category == "C")
            catD = questionsFromJson.filter(question => question.question_category == "D")
            catE = questionsFromJson.filter(question => question.question_category == "E")
            ...
            res.render('index', { question: questionsByCategory[0][0] });
        }
    ).catch(
        (err) => {
            console.log(err);
...
app.post('/nextQuestion', (req, res) => {
    let currentQuestionIndex = questionsByCategory[currentCategoryIndex].indexOf(currentQuestion, 0);
    ...
    if (req.body.answer === currentQuestion.question_answers[currentQuestion.right_answer_index]) {
        currentQuestionIndex += 1;
        strikeAnswersCount += 1;
        incrementRightAnswers(currentQuestion);
        if (currentQuestionIndex > questionsByCategory[currentCategoryIndex].length - 1) {
            res.render("finish", {
                rightA: (rightAnswersFromA / questionsByCategory[currentCategoryIndex].length) * 100,
                rightB: (rightAnswersFromB / questionsByCategory[currentCategoryIndex].length) * 100,
                rightC: (rightAnswersFromC / questionsByCategory[currentCategoryIndex].length) * 100,
                rightD: (rightAnswersFromD / questionsByCategory[currentCategoryIndex].length) * 100,
                rightE: (rightAnswersFromE / questionsByCategory[currentCategoryIndex].length) * 100,
            });
        }
        if (strikeAnswersCount == 3) {
            ...
            currentQuestion = questionsByCategory[currentCategoryIndex][currentQuestionIndex];
            res.render('index', { question: currentQuestion });
        }
    } else {
        ...
        if (strikeAnswersCount - 1 > 0 & strikeAnswersCount - 1 < 3) {
            currentQuestionIndex += 1;
            // strikeAnswersCount += 1;
            currentQuestion = questionsByCategory[currentCategoryIndex][currentQuestionIndex];
            res.render('index', { question: currentQuestion });
...
function switchCategory(currentQ, currentQI) {
    ...
    let currentQuestionCategory = questionsByCategory[currentCategoryIndex][0].question_category;

    if (currentQI > questionsByCategory[currentCategoryIndex].length - 1) {
        res.render('finish')
    } else {
        currentQuestion = questionsByCategory[currentCategoryIndex][getRightQuestionIndex(currentQuestionCategory)];
